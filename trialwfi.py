...
future_data = pd.DataFrame(
    {"gdp_per_capita_usb": [future_gdp], "military_pct_gdp": [future_pct]}
)
...
gdp_growth_rate = (
    training_data["gdp_per_capita_usb"].iloc[-1]
    - training_data["gdp_per_capita_usb"].iloc[0]
) / 13
military_troops_growth = (
    latest_year_data["military_troops"] - training_data["military_troops"].iloc[0]
) / 13
...
army_strength = int(
    project_future_value(latest_year_data["military_troops"], military_troops_growth)
)

fighter_sorties = int(
    project_future_value(
        latest_year_data["fighter_active"],
        (latest_year_data["fighter_active"] - training_data["fighter_active"].iloc[0])
        / 13,
    )
)
