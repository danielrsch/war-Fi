...
parser.add_argument(
    "--clear-history",
    action="store_true",
    help="Delete the local simulation_history.json file",
)
...
historical_data = pd.DataFrame(
    {
        ...
        "Total_Military_Spending_Billions": [
            738,
            752,
            725,
            680,
...
plt.scatter(
    ...
    historical_data[target_budget] - predicted_training_budget,
    color="red",
)
...
projected_defaults = {
    col: calculate_future_trend(col, 2025)
    for col in [
        "GDP_Per_Capita",
        "Military_Percent_Of_GDP",
        "Inflation_Rate",
...
def get_user_input(prompt_text, default_value, min_limit=0.0, max_limit=1e15):
    ...
    try:
        ...
        print(
            f"  [!] Invalid or out-of-bounds number. Using default: {default_value:.2f}"
        )
        return default_value


user_gdp = get_user_input(
    "Estimated GDP Per Capita in USD", projected_defaults["GDP_Per_Capita"]
)
user_mil_pct = get_user_input(
    "Military Spending as % of GDP", projected_defaults["Military_Percent_Of_GDP"]
)
user_inflation = get_user_input(
    "Estimated Inflation Rate (%)", projected_defaults["Inflation_Rate"]
)
user_aid = get_user_input(
    "Foreign Military Aid (Billions USD)", projected_defaults["Foreign_Aid_Billions"]
)
...
print("=" * 50)
print(
    f"Predicted Total Military Spending = {format_large_number(predicted_spending_usd)} (${predicted_spending_usd:,.0f})"
)
